# Event V3 Q16/Q18/Q19/Q20 Query-only Runbook

## Scope
- Endpoints:
  - Q16: `/api/event/v3/characters/{characterId}/rise`
  - Q18: `/api/event/v3/events/{eventId}/perspectives`
  - Q19: `/api/event/v3/dramas/{dramaId}/conflict-axes`
  - Q20: `/api/event/v3/characters/{characterId}/narrative-distribution`
- Runtime switches:
  - `EVENT_V3_Q16_SOURCE_MODE` = `rdb | rdf-candidate | auto-fallback`
  - `EVENT_V3_Q18_SOURCE_MODE` = `rdb | rdf-candidate | auto-fallback`
  - `EVENT_V3_Q19_SOURCE_MODE` = `rdb | rdf-candidate | auto-fallback`
  - `EVENT_V3_Q20_SOURCE_MODE` = `rdb | rdf-candidate | auto-fallback`
...
EVENT_V3_Q18_SOURCE_MODE=auto-fallback \
EVENT_V3_Q19_SOURCE_MODE=auto-fallback \
EVENT_V3_Q20_SOURCE_MODE=auto-fallback \
...
EVENT_V3_Q18_SOURCE_MODE=rdb \
EVENT_V3_Q19_SOURCE_MODE=rdb \
EVENT_V3_Q20_SOURCE_MODE=rdb \
docker compose -f infra/docker-compose.yml --env-file .env \
  up -d --no-build --no-deps --force-recreate event-service
```
...
# Q18
curl -sS "http://localhost:8080/api/event/v3/events/100/perspectives?safeUpToEpisode=6"

# Q19
curl -sS "http://localhost:8080/api/event/v3/dramas/10/conflict-axes?safeUpToEpisode=6&limit=10"

# Q20
curl -sS "http://localhost:8080/api/event/v3/characters/17/narrative-distribution?dramaId=10&safeUpToEpisode=6"
```

## Log Check
```bash
docker logs nospoiler-event-service --tail 300 \
  | rg "eventV3.q16 sourceMode|eventV3.q16.shadow|eventV3.q18 sourceMode|eventV3.q18.shadow|eventV3.q19 sourceMode|eventV3.q19.shadow|eventV3.q20 sourceMode|eventV3.q20.shadow"
```
...
- shadow parity line:
  ...
  - Q18: `statusParity`, `perspectiveParity`, `evidenceJaccard`
  - Q19: `statusParity`, `axisParity`, `evidenceJaccard`
  - Q20: `statusParity`, `distributionParity`, `evidenceJaccard`
