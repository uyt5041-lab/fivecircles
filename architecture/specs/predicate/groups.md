...
| BATTLE | ATTACKS, DEFEATS, KILLS | BATTLE, CONFRONTS | "전투"의 최소 근사. 필요 시 CAPTURES 등 추가 검토. |
| ADVERSARY | CAPTURES, BETRAYS | THREAT, THREATENS, THREATENED, INTIMIDATES, COERCES, BLACKMAIL, MANIPULATES, SEIZES_CONTROL, DOMINATES, POWER_SHIFT, TRUST_COLLAPSE, RELATIONSHIP_DAMAGE | 파생 질문(적대자). BATTLE과 중복 카운팅을 피하려고 공격/전투는 BATTLE로만 집계. |
| ALLY | ALLIES_WITH | ALLY, ALLIES_WITH, PARTNERS_WITH, CO_CONSPIRATOR, SUPPLY_CONTRACT, RECURRING_DEAL | 파생 질문(협력자). AFFILIATION_CHANGE(JOINS/LEAVES)와 중복을 피하려고 JOINS는 제외. |
