...
| `Q02` | 월터가 암페타민 제조시작한게 언제지? | `character_keyword_earliest` | `S1E1` | `subject=Walter`, `qAnyOf=[처음으로 메스암페타민,직접 제조]` | Walter keyword broad(정답 확정 금지) | `HIDE_EXISTS_BEYOND_K` | `2285` |
...
| `Q04` | 스카일러가 남편의 범죄사실을 알아차린 시점이언제냐? | `character_predicate_earliest` | `S3E2` | `subject=Skyler`, `predicateCodeAnyOf=[DISCOVERS,LEARNS]`, `qAnyOf=[meth,메스,암페타민,마약,제조,범죄]` | `predicateCodeAnyOf=[DISCOVERS,LEARNS]` | `HIDE_EXISTS_BEYOND_K` | `TBD` |
...
| `Q07` | 월터가 처음 거짓말을 들키는 순간? | `character_predicate_earliest` | `S2E2` | `subject=Walter`, `qAnyOf=[Which one,어느 폰,두 번째 폰,두 번째 휴대폰]` | `qAnyOf=[거짓말,부정,변명,두 번째 폰]` | `HIDE_EXISTS_BEYOND_K` | `3001` |
...
| `Q14` | 스카일러-월터 관계가 돌이키기 어려워지는 첫 균열? | `coevents_earliest` | `S2E13` | `with=[Walter,Skyler]`, `predicateCodeAnyOf=[BETRAYS,LEARNS,DISCOVERS]`, `qAnyOf=[별거,신뢰 붕괴,집에서 나가,집에서 내보내]` | `predicateCodeAnyOf=[MEETS]` | `HIDE_EXISTS_BEYOND_K` | `2923` |
