...
|---|---|---|---|
| Q05 | `LEARNS/DISCOVERS` + `결심/동업/제조 시작/암 진단` | `DEA`, `단속`, `도주`, `제시` | S1E1의 “범죄 진입 계기” 이벤트 매칭 가능성 증가 |
| Q07 | `DISCOVERS/LEARNS` + `거짓말/휴대폰/실종/추궁` | `의심`, `행동 이상`, `불신` | “들킴/의심” 이벤트와 키워드 교집합 확보 |
| Q11 | `target=Walter` + `DISCOVERS/LEARNS` + `의심/휴대폰/실종` | `의심`, `불안`, `행동 이상`, `불신` | target 유지 시 교집합 후보 확대(0건 완화 여부 확인 필요) |
| Q12 | `ATTACKS/DEFEATS` + `폭발/협상/통제권` | `투코`, `협박`, `주도권` | ATTACKS 계열과 권력 전환 의미를 동시에 포착 시도 |
