...
- [ ] **개발자 전용 보안 기능 요구사항 정리**: 잠금 처리된 정보는 일반 사용자에게 노출되지 않고 개발자만 확인할 수 있는 보호 기능을 정의한다.
  - [ ] 사용자 공개 영역과 개발자 전용 영역을 구분하는 기준 정리
  - [ ] 잠금 상태에서 어떤 정보가 숨겨지고 어떤 권한에서만 보이는지 정책 정의
  - [ ] 개발자 전용 확인 절차와 오노출 방지 규칙 정리
  - [ ] 문서/설정/운영 화면에 공통 적용 가능한 보안 가시성 원칙 초안 작성

- [ ] **레포 민감정보 및 보안 제거 항목 점검**: 저장소에 포함되면 안 되는 민감정보와 보안상 제거해야 할 요소를 점검하고 정리 기준을 만든다.
  - [ ] 토큰, 키, 계정 정보, 내부 경로, 개인 식별 정보 등 민감정보 점검 항목 정의
  - [ ] 새 프로젝트로 이식할 때 제거해야 하는 보안 민감 흔적 목록화
  - [ ] 커밋 전/배포 전 점검해야 할 보안 체크리스트 초안 작성
  - [ ] 문서/스크립트/설정 파일별로 우선 확인해야 할 위험 포인트 정리

- [ ] **보안 고도화 후속 작업 정리**: 완벽에 가깝게 가기 위한 보안 후속 작업을 체계화한다.
  - [ ] Git 히스토리 전체를 대상으로 민감정보 재스캔
  - [ ] 시크릿 스캔 도구 도입 검토 및 적용 방식 정리
  - [ ] pre-commit / CI에 민감정보 검사 추가
  - [ ] 배포/테스트용 예시값을 전부 플레이스홀더화
  - [ ] 민감정보 탐지 실패 시 대응 절차와 정리 순서 문서화

- [ ] **gitignore 기반 보안/운영 모드 스킬 설계**: 문서/로그 민감정보를 수작업 규칙만으로 막지 않고 `gitignore` 기반 모드 전환으로 제어하는 스킬을 설계한다.
  - [ ] 옵션 1: `fivecircles` 폴더 전체 ignore
  - [ ] 옵션 2: 민감정보성 문서/로그만 선택 ignore
  - [ ] 옵션 3: ignore 비활성화
  - [ ] 모드 전환 시 어떤 파일이 추적되고 해제되는지 규칙 정의
  - [ ] 새 프로젝트 이식 시 기본 모드를 무엇으로 둘지 정책 결정
  - [ ] 스킬 명령으로 모드를 켜고 끄는 UX 초안 작성
