...
- Maintain project documentation under `fivecircles/` (excluding scoring):
 ...
 - **Role-bound logging**: Only log and update todos within the assigned role's scope.
 - **Assigned role**: Team C (event/policy/qa services). Do not log Team A/B work.
...
- Do not add todos or error logs for Team A/B work; if a non‑C failure blocks execution, do not log it.
