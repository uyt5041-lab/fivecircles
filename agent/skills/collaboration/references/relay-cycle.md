...
relay-shot에서 진입하면 협업이 기본이며, 해당 실행에서 사용자가 명시한 solo 요청만 예외다.
메인이 선택한 모드를 후속 배치·원샷딜·시간 제한 인계에도 유지한다.
협업 모드 선택은 커밋·push·범위 확대 승인이 아니다.
...
- 중간 검수의 시작 조건·깊이는 협업 스킬의 검수 시점 표를 따른다. 3개 leaf/30분은
  위험도 재확인 시점이며 자동 Critic 호출 조건이 아니다. 필수 조건 없는 3점 미만은 WORK를 유지한다.
- 중간 검수 PASS 후 배치가 남아 있으면 기존 DIRECTIVE의 남은 범위로 WORK를 재개한다.
  최종 후보의 유효한 PASS와 필수 검증·완료 조건이 갖춰져야 READY_TO_INTEGRATE로 간다.
